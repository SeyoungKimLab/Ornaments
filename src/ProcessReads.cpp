...
void MasterProcessor::update(const std::vector<int>& c, const std::vector<std::vector<int> > &newEcs, const std::vector<variant_class>& newVEcs, const std::unordered_map<int, int>& aseCounts,
...
//  std::cerr << std::endl << "We are updating" << std::endl;
  if (!opt.batch_mode) {
    ...
    // we need to update this, but the size of tc.aseCounts is unknown
    nummapped += newVEcs.size();
    for (auto& pair : newVEcs) {
      int new_vc = tc.increaseVCount(pair);
    }

    for (auto& kv : aseCounts) {
      int vc = kv.first;
      int count = kv.second;
      tc.aseCounts[vc] += count;
      nummapped += count;
    }
  } else {
    if (!opt.umi) {
  ...
  if (!opt.batch_mode) {
//    std::cerr << "We are in fact here GRIME?" << std::endl;
    for(auto &u : newEcs) {
//      std::cerr << "And here, the u vector is:" << std::endl;
//      std::cerr << u << std::endl;
      ++newECcount[u];
    }
  } else {
...
//    std::cerr << "yes? god damn it" << std::endl;
...
void ReadProcessor::operator()() {
  while (true) {
    ...
    mp.update(counts, newEcs, newVEcs, aseCounts, ec_umi, new_ec_umi, paired ? seqs.size()/2 : seqs.size(), flens, bias5, pseudobatch, tmp_v, std::vector<std::pair<BUSData, std::vector<int32_t>>>{}, nullptr, nullptr, id, local_id);
    clear();
  }
}
...
void ReadProcessor::processBuffer() {
  ...
  std::unordered_map<int, variant_info> vi1, vi2;

  u.reserve(1000);
  v1.reserve(1000);
  v2.reserve(1000);
  vtmp.reserve(1000);
  std::vector<int> num_eff_vs;
  ...
  // actually process the sequences
    ...
    auto new_end1 = std::remove_if(
        v1.begin(),
        v1.end(),
        [&](std::pair<KmerEntry, int> a)
        {
          int tec = index.dbGraph.ecs[a.first.contig];
          return index.ecmap[tec].size() == 0;
        });
    v1.erase(new_end1, v1.end());

    if (paired) {
      index.match(s2,l2, v2);
      auto new_end2 = std::remove_if(
          v2.begin(),
          v2.end(),
          [&](std::pair<KmerEntry, int> b)
          {
            int tec = index.dbGraph.ecs[b.first.contig];
            return index.ecmap[tec].size() == 0;
          });
    ...
    std::vector<int> flagForRemoval;
    
    for (int t : u) {
      // if this is in here, then its parent sequence is here too. delete it out the u vector
      if (index.target_names_[t].find("_refsnp_") != std::string::npos) {
        flagForRemoval.push_back(t);
      }
    }

    for (int f : flagForRemoval) {
      u.erase(std::remove(u.begin(), u.end(), f), u.end());
    }

    /* Want to remove all spurious shades from vi1 and vi2.
     * Spurious shades are defined as shades whose host color is not present in the final pseudoalignment */
    

    tc.unionShades(v1, vi1, u, variantU, s1);
    std::vector<int> effectiveParams;
    for (auto& itr : vi1) {
      // itr.first is the reference transcript for the variant (trid)
      // itr.second is a map for this transcript
      // 'r' contains snps for which this read aligned ref, 'a' contains snps for which this read aligned alt
      // Only search add variants if they exist in transcriptToParam map, which is specific to individual
      auto& variantInfo = itr.second;
      for (int rsnp : std::get<0>(variantInfo)) {
        auto s1 = index.transcriptToParam.find(itr.first);
        if (s1 != index.transcriptToParam.end()) {
          auto& map = s1->second;
          auto s2 = map.find(rsnp); // s2 maps the snp in a transcript to its effective parameter
          if (s2 != map.end()) {
            /* std::cerr << "get in here? " << std::endl; */
            int param_index = s2->second;
            auto& vpeList = index.effectiveASEParams.at(param_index);
            int referrer = 0;
            for (auto& vpe : vpeList) {
              if (vpe.trid == itr.first && vpe.pos == rsnp) {
                param_index *= (vpe.orientation == 'r' ? 1 : -1);
                break;
              }
              referrer++;
            }
            if (std::find(effectiveParams.begin(), effectiveParams.end(), param_index) == effectiveParams.end()) {
              // if this matches the orientation of the parameter, add as positive
              // else, add as negative one times this value
              effectiveParams.push_back(param_index);
            }
          } 
        } 
      }

      for (int asnp : std::get<1>(variantInfo)) {
        auto s1 = index.transcriptToParam.find(itr.first);
        if (s1 != index.transcriptToParam.end()) {
          auto& map = s1->second;
          auto s2 = map.find(asnp);
          if (s2 != map.end()) {
            int param_index = s2->second;
            auto& vpeList = index.effectiveASEParams.at(param_index);
            int referrer = 0;
            for (auto& vpe : vpeList) {
              if (vpe.trid == itr.first && vpe.pos == asnp && vpe.is_first) {
                param_index *= (vpe.orientation == 'a' ? 1 : -1);
                break;
              }
              referrer++;
            }
      ...
      for (int asnp : std::get<2>(variantInfo)) {
        auto s1 = index.transcriptToParam.find(itr.first);
        if (s1 != index.transcriptToParam.end()) {
          auto& map = s1->second;
          auto s2 = map.find(asnp);
          if (s2 != map.end()) {
            int param_index = s2->second + (mp.opt.phased ? 0 : 1);
            /* Multiallelic site should always occur right after the occurrence of the first allele, since the input vcf file is sorted */
            if (index.effectiveASEParams.size() <= param_index) continue;
            auto& vpeList = index.effectiveASEParams.at(param_index);
            int referrer = 0;
            for (auto& vpe : vpeList) {
              if (vpe.trid == itr.first && vpe.pos == asnp && !vpe.is_first) {
    ...
    if (paired) {
      tc.unionShades(v2, vi2, u, variantU, s2);
      for (auto& itr : vi2) {
        // itr.first is the reference transcript for the variant (trid)
        // itr.second is a map for this transcript
        // 'r' contains snps for which this read aligned ref, 'a' contains snps for which this read aligned alt
        ...
        auto& variantInfo = itr.second;
        for (int rsnp : std::get<0>(variantInfo)) {
           // std::cerr << std::endl << "ref snp: " << rsnp << std::endl;
          auto s1 = index.transcriptToParam.find(itr.first);
          if (s1 != index.transcriptToParam.end()) {
            auto& map = s1->second;
            auto s2 = map.find(rsnp); // s2 maps the snp in a transcript to its effective parameter
            if (s2 != map.end()) {
              ...
              for (auto& vpe : vpeList) {
                if (vpe.trid == itr.first && vpe.pos == rsnp) {
                  param_index *= (vpe.orientation == 'r' ? 1 : -1);
                  /* If in phased mode, only ornaments with consistent phase will be here */
                  break;
                }
                referrer++;
              }
    ...
    if (!mp.opt.single_overhang && !mp.opt.umi && !u.empty() && (!paired || v1.empty() || v2.empty()) && tc.has_mean_fl) {
      ...
      for (auto tr : u) {
        ...
        /* TODO: This should actually depend on the strandedness of the read. */ 
          vtmp.push_back(tr);
        } else {
          //pass
        }
        // with an exception for if an ornament for this transcript was also here
    ...
    std::vector<int> retainedParams;
    if (!effectiveParams.empty()) {
      for (int p : effectiveParams) {
        int ind_p = p > 0 ? p : -1 * p;
        auto& vpeList = index.effectiveASEParams.at(ind_p);
        if (std::find(u.begin(), u.end(), vpeList[0].trid) != u.end()) {
          retainedParams.push_back(p);
        }
      }
      std::sort(retainedParams.begin(), retainedParams.end());
    }

    flagForRemoval.clear();
    for (int p : retainedParams) {
      if (std::find(retainedParams.begin(), retainedParams.end(), -1 * p) != retainedParams.end()) {
        // remove incongruent transcripts, since both phase ornaments were found
        if (std::find(flagForRemoval.begin(), flagForRemoval.end(), p) == flagForRemoval.end()) {
          int ind_p = p > 0 ? p : -1 * p;
          auto vpe_list = index.effectiveASEParams.at(ind_p);
          int trid = vpe_list[0].trid;
          flagForRemoval.push_back(p);
          u.erase(std::remove(u.begin(), u.end(), trid), u.end());
        }
      }
    }

    for (int f : flagForRemoval) {
      retainedParams.erase(std::remove(retainedParams.begin(), retainedParams.end(), f), retainedParams.end());
    }
    ...
    if (!u.empty()) {
      ...
      // if this vector is empty, then use the old ec formulation
      // else, use the new counts objects
      if (!retainedParams.empty()) {
        variant_class p = std::pair<std::vector<int>, std::vector<int>>(u, retainedParams);
        int vc = tc.findVC(p);
        if (vc != -1) {
          ++aseCounts[vc];
        } else {
          /* Every single VC that is formed comes through here originally */
          newVEcs.push_back(p);
        }
      }
      else { // I think this breaks certain run modes?
        if (!mp.opt.umi) {
...
void ReadProcessor::clear() {
  ...
  /* aseCounts.resize(tc.aseCounts.size(), 0); */
  ec_umi.clear();
  new_ec_umi.clear();
}

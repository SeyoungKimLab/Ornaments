...
typedef std::pair<std::vector<int>, std::vector<int>> variant_class;
typedef std::tuple<std::vector<int>, std::vector<int>, std::vector<int>> variant_info;
...
struct MinCollector {
  ...
  int increaseVCount(const variant_class& p);
  int decreaseCount(const int ec);

  std::vector<int> intersectECs(std::vector<std::pair<KmerEntry,int>>& v) const;
  /* Instead of treating this as a map of char to int, just treat as a pair where the left is r and the right is a */
  void unionShades(std::vector<std::pair<KmerEntry,int>>& v,
                   std::unordered_map<int, variant_info>& variantInfo,
                   std::vector<int>& u,
                   std::unordered_map<int, int>& variantU,
                   const char* s) const;
  int isUniqueOrCompatible(
  ...
  int findVC(const std::pair<std::vector<int>, std::vector<int>>& p) const;
  ...
  std::unordered_map<int, int> aseCounts;
  std::unordered_map<int, double> effParamsBias;

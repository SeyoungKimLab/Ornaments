...
void ParseOptionsEM(int argc, char **argv, ProgramOptions& opt) {
  ...
  const char *opt_string = "t:i:l:s:o:n:m:d:b:g:c:v:a:r:q:w";
  static struct option long_options[] = {
    // long args
    {"verbose", no_argument, &verbose_flag, 1},
    ...
    {"phased", no_argument, &phased_flag, 1},
    ...
    {"sample", required_argument, 0, 'a'},
    {"repeat", required_argument, 0, 'r'},
    {"conv", required_argument, 0, 'q'},
    {0,0,0,0}
  };
  int c;
  int option_index = 0;
  while (true) {
    ...
    case 'v': {
      stringstream(optarg) >> opt.vcf_file;
      break;
    }
    case 'a': {
      stringstream(optarg) >> opt.sample_name;
      break;
    }
    case 'q': {
      stringstream(optarg) >> opt.conv;
      break;
    }
  ...
  if (argc < 2) {
    ...
    if (cmd == "version") {
      ...
      if (!CheckOptionsEM(opt)) {
        ...
        time(&end);
        std::cerr << "Pseudoalignment took " << difftime(end, start) << " seconds." << std::endl;
...
//        std::cerr << "In main.cpp now" << std::endl;
//        for (int ec = 0; ec < collection.counts.size(); ec++) {
//          if (collection.counts[ec] > 0) {
//            for (int t : index.ecmap[ec]) {
//              std::cerr << index.target_names_[t] << ", ";
//            }
//            std::cerr << std::endl;
//          }
//        }
//        std::cerr << "Exiting main.cpp now" << std::endl;

        EMAlgorithm em(collection.counts, collection.aseCounts, index, collection, fl_means, opt);

        time(&start);
        em.run(10000, 50, true, opt.bias);
        time(&end);
        std::cerr << "EM took " << difftime(end, start) << " seconds." << std::endl;
        ...
        for (auto& kv : collection.aseCounts) {
          num_pseudoaligned += kv.second;
        }

...
public:
  ...
  void update(const std::vector<int>& c, const std::vector<std::vector<int>>& newEcs, const std::vector<variant_class>& newVEcs, const std::unordered_map<int, int>& aseCounts, std::vector<std::pair<int, std::string>>& ec_umi, std::vector<std::pair<std::vector<int>, std::string>> &new_ec_umi, int n, std::vector<int>& flens, std::vector<int> &bias, const PseudoAlignmentBatch& pseudobatch, std::vector<BUSData> &bv, std::vector<std::pair<BUSData, std::vector<int32_t>>> newB, int *bc_len, int *umi_len,   int id = -1, int local_id = -1);
};
...
public:
  ...
  std::vector<variant_class> newVEcs;
